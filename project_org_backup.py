...
class Calendar:
    ...
    def update_calendar_data(self: object, data):
    ...
    while True:
        ...
        if user_input == 1:
            ...
            while True:
                ...
                print(f"u_input {user_input_dialogue}")
...
def create_new_calendar():
    ...
    save_menu = create_menu(
        "Save", ["Save calendar", "Read entry", "Back to main menu"]
    )
    save_menu.display_menu()
...
def get_calendar_input(adCalendar: object):
    ...
    cal_name = "What do you want to call this calendar? "
    cal_year = "For which year do you want to generate a calendar? "
    cal_month = "For which month do you want to generate a calendar? "

    while True:
        adCalendar.name = input(cal_name)
        cal_name_duplicate = f"A calendar with the name '{adCalendar.name}' already exists. Please choose a different name."
        if not Calendar.is_calendar_name_unique(adCalendar.name):
            print(cal_name_duplicate)
            continue  # Go back to the start of the loop
        else:
            break

    year = adCalendar.get_valid_input(cal_year, "year")
    month = adCalendar.get_valid_input(cal_month, "month")

    cal_custom_day_selector = f"Do you want to use the default amount of days for this month ({get_days_month(year, month)}) [Y/n]? "
    cal_custom_days = f"For how many days do you want to run the calendar in {month}? "
